Hyrax.config do |config|
  # Injected via `rails g hyrax:work GwWork`, etc.
  ENV["CURATION_CONCERNS"].split(",").each do |concern|
    config.register_curation_concern(concern.to_sym)
  end
  ...
  config.temp_file_base = ENV["TEMP_FILE_BASE"]
  ...
  # This is only relevant if the minter_class (above) is set to file
  if ENV["MINTER_STATEFILE"] 
    config.minter_statefile = ENV["MINTER_STATEFILE"]
  end
  ...
  config.geonames_username = ENV["GEONAMES_USERNAME"] # insert username here
  ...
  config.banner_image = ENV["BANNER_IMAGE"]
  ...
  config.iiif_image_url_builder = lambda do |file_id, base_url, size, format| end
